...
### 标题要求
- 一篇文章只能有一个一级标题（`#`），且必须是文章的标题。其他内容应使用二级标题（`##`）或更低级别的标题。
- 在文件中，正文应不出现一级标题（`#`），文章标题只需在 Front-matter 的 `title` 字段中指定即可。
...
### 格式要求
- 标题均使用数字序号开头，且下一级标题应在上一集标题数字序号的基础上增加小数点，如：
  - `## 1 一级标题`
  - `### 1.1 二级标题`
  - `### 1.2 二级标题`
  - `#### 1.2.1 三级标题`
...
- 如果文章中有一级标题（`#`），在生成 Front-matter 时应被删去，并将其内容放入 `title` 字段中。
- 中文和西文字符间应当用空格分隔。
