...
此处的 `--init` 参数相当于先执行 `init` 子命令后再执行 `update` 子命令，`git submodule init` 会根据 `.gitmodules` 文件的信息初始化本地 Git 配置（`.git/config`）。
...
`sync` 子命令用于将 `.gitmodules` 文件中定义的远程 URL 同步到本地 Git 配置（`.git/config`）中。
...
### 2.4 `.git/modules/<路径>`：子模块实际的 Git 数据
