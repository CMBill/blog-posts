...
GitHub Copilot 学生套餐一改再改，现如今删除了大部分强大的模型不说，一个月还只有相当于 2 美元的额度，基本已经属于不可用的程度了。现在也有越来越多的独立于编辑器的编码智能体，如 Codex、OpenCode 等，但是依旧可以利用插件，继续在 VSCode 里以习惯的方式使用其他供应商提供的模型（如 DeepSeek）。
